    ...
    public void onRecvPipeData(short paramShort, XDevice paramXDevice, byte[] paramArrayOfByte)
    {
      ...
      if ((str1.length() == 196) && (str1.indexOf("3BEB") != -1) && (TimingBussines.this.isCreateNewTimeOk))
      {
        ...
        for (int i2 = 0; i2 < localList.size(); i2++)
          System.out.println(TimingBussines.this.tTag + "MyVo = ---------------     " + ((TimingVo)localList.get(i2)).getXuHao() + "     ------------" + ((TimingVo)localList.get(i2)).isOther());
        ...
        for (int i5 = 0; i5 < localArrayList2.size(); i5++)
        {
          ...
          {
            TimingVo localTimingVo2 = (TimingVo)localList.get(i6);
      ...
      if (str1.length() == 20)
      {
        ...
        if ((i | j) != 0)
        {
          ...
          localHandler2.postDelayed(local1, 300L);
        }
        if (str1.length() != 18)
          break label1723;
        k = 1;
        label1532: if (str1.indexOf("A4EB") == -1)
          break label1729;
      }
      label1717: label1723: label1729: for (int m = 1; ; m = 0)
      {
        if ((k | m) != 0)
        {
 ...
 * Qualified Name:     com.ex.ltech.ct.timing.TimingBussines
